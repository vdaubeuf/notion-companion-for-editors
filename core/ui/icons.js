...
// Hosts whose renderer cannot draw inline SVG reliably (UXP) add the class
// "glyph-icons" on <body>: text glyphs are used instead.
...
function useGlyphs() {
    return !!(document.body && document.body.classList.contains('glyph-icons'));
}

function icon(name, cls = '') {
    if (useGlyphs()) {
        const span = document.createElement('span');
        span.className = `icon glyph ${cls}`.trim();
