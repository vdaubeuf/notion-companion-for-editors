...
class NotionClient {
    ...
    async request(method, path, { query, body, signal, token } = {}) {
        ...
        if (query) {
            // Built by hand: URLSearchParams is not available in every host runtime (UXP).
        ...
        for (let attempt = 0; ; attempt += 1) {
            ...
            let timedOut = false;
            let timer;
            // Hard timeout that does not rely on the runtime honouring AbortSignal.
            const timeout = new Promise((_, reject) => {
                timer = setTimeout(() => { timedOut = true; onAbort(); reject(new Error('timeout')); }, this.timeoutMs);
            });
            try {
                res = await Promise.race([timeout, this.fetch(url, {
                    method,
                    headers: {
                ...
                else if (timedOut) error = new NotionError('timeout');
