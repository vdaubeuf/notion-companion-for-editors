...
export class SearchView {
    ...
    constructor(opts) {
        ...
        this.input = h('input', {
            class: 'search-input', type: 'search', placeholder: 'Rechercher une page Notion…', autocomplete: 'off', spellcheck: 'false',
        ...
        this.el = h('div', { class: 'view search-view' },
            h('div', { class: 'view-head' },
                h('button', { class: 'icon-btn', title: 'Retour', 'aria-label': 'Retour', onClick: opts.onClose }, icon('back')),
    ...
    _markActive() {
        [...this.list.querySelectorAll('.result')].forEach((el, i) => el.classList.toggle('active', i === this.active));
    }
    ...
    _render() {
        ...
        const pasted = idFromInput(this.input.value);
        if (pasted) {
            this.list.appendChild(h('button', { class: 'result pasted', onClick: () => this._pick({ id: pasted, title: 'Page (lien collé)' }) },
        ...
        this.results.forEach((r, i) => this.list.appendChild(this._row(r, i)));
        ...
        if (this.loading) {
            ...
            this.footer.appendChild(h('button', { class: 'btn small ghost wide', onClick: () => this._run(true), text: 'Plus de résultats' }));
        }
    }
}
