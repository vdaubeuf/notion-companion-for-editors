...
// Orchestrates Resolve project state -> association -> Notion content.
// Emits:
//   'state'   small UI state object (safe to send to the renderer)
...
class Controller extends EventEmitter {
    constructor({ watcher, associations, secrets, cache, client, log }) {
        super();
        ...
        this.state = {
            resolve: { status: 'connecting', project: null, resolveVersion: null, uidSupported: null },
        ...
        watcher.on('change', (ws) => this._onResolveState(ws));
    }
    ...
    _onResolveState(ws) {
        const resolve = { status: ws.status, project: ws.project, resolveVersion: ws.resolveVersion, uidSupported: ws.uidSupported };

        // Transient unavailability: keep showing the last project and its page.
        if (ws.status !== 'ok' && ws.status !== 'no_project') {
            this._set({ resolve: { ...resolve, project: this.state.resolve.project } });
            return;
        }
    ...
    _applyAssociation() {
        ...
        const sugg = suggestion ? {
            key: suggestion.key,
            projectName: suggestion.resolveProjectName,
            folder: suggestion.resolveFolder || null,
    ...
    async _loadPage(pageId, { preferNetwork = false } = {}) {
        ...
        const cached = preferNetwork ? null : this.cache.get(pageId);
        if (cached) {
    ...
    // ---------- public actions (called through IPC) ----------
    ...
    listAssociations() {
        return this.associations.list().map((a) => ({
            key: a.key,
            projectName: a.resolveProjectName,
            database: a.resolveDatabase ? a.resolveDatabase.name : null,
            folder: a.resolveFolder || null,
            strategy: a.resolveProjectUid ? 'uid' : 'fallback',
            pageId: a.notionPageId,
