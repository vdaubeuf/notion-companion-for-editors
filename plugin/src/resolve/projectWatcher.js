...
class ProjectWatcher extends EventEmitter {
    constructor({ bridge, intervalMs, unavailableIntervalMs, log }) {
        ...
        this.running = false;
        this.inFlight = false;
        this.state = { status: 'connecting', project: null, resolveVersion: null, uidSupported: null };
    ...
    async _snapshot() {
        ...
        if (!this.state.resolveVersion) {
            const v = await safeCall(resolve, 'GetVersionString');
            if (v.ok && v.value) this.state.resolveVersion = String(v.value);
        }
